...
class CycleGANCrossEntTrainer(BaseTrainer):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.losses['G_A-CE'] = 0.0
            self.losses['G_A-GAN'] = 0.0
    ...
    def _set_image_pool(self):
        self.image_pool['fake_B'] = ImagePool(self.opt.pool_size, self.opt.batchSize)
        self.image_pool['fake_A'] = ImagePool(self.opt.pool_size, self.opt.batchSize)
        self.image_pool['real_B'] = ImagePool(self.opt.pool_size, self.opt.batchSize)
    ...
    def backward_D_B(self):
        ...
        fake_B = self.image_pool['fake_B'].query(self.fake_B)

        if self.use_real_B:
            # one-hot real_B (or + noise), dtype=Float
            self.compute_real_B_onehot(fake_B)
            real_B_onehot = self.image_pool['real_B'].query(self.real_B_onehot)
        else:
            real_B_onehot = self.image_pool['real_B'].query()

        # D_B(B)
        if real_B_onehot is not None:
    ...
    def backward_D_A(self):
        rec_A = self.image_pool['fake_A'].query(self.rec_A)
